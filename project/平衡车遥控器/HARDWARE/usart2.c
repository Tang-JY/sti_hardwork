...
void usart2_SendSync(void)
{
	...
	const u8 head[2] = {0xde,0xad};//同步数据包数据头dead
	
	if(memcmp( (char*)&temp , (char*)dataSync_TX, sizeof(DataSync_TX_TypeDef) ))//若数据被修改过，才发送同步数据
	{
		memcpy(TX2_in,head,2);
		TX2_in += 2;
		memcpy(TX2_in,(char*)dataSync_TX,sizeof(DataSync_TX_TypeDef));
		TX2_in += sizeof(DataSync_TX_TypeDef);
		USART2->CR1|=(0x0001<<7);//传输开始，打开TXE中断
		
		memcpy((char*)&temp  ,(char*)dataSync_TX,sizeof(DataSync_TX_TypeDef));//保存本次数据以便下次比对
		PRINT(" usart2:已发送同步\r\n");
	}
}
...
void USART2_IRQHandler(void)//串口2中断服务程序
{
	...
	static u8 temp[sizeof(DataSync_RX_TypeDef)]={0};
	static u8 i=0;
	u8 Rec;
			...
			default:
				...
				if(i<sizeof(DataSync_RX_TypeDef) && state==2){
					...
					if(i==sizeof(DataSync_RX_TypeDef)){//若数据同步完毕					
						PRINT(" usart2:已接收同步\r\n");
						memcpy((char*)dataSync_RX,temp,sizeof(DataSync_RX_TypeDef));
						i=0;
						state=0;
					}
				}
