...
void UC1701DataWrite(unsigned char ucData)
{
   ...
   //Step 1 Select Chip 片选
   GPIOPinWrite(LCD_PIN_SPI_CS,GPIO_PIN_5,0<<5);
    
   //Step 2 Set Display Enable (CD表示Control/Data,此处选择Data)
   ...
   //Step 3 Disable chip select 取消片选
    GPIOPinWrite(LCD_PIN_SPI_CS,GPIO_PIN_5,1<<5);
 }
...
//
 ...
 void UC1701CmdWrite(unsigned char ucCmd)
 {
   ...
   //Step 2 Send a command (CD表示Control/Data,此处选择Control)
...
void UC1701Dispaly(unsigned char ucLine, unsigned char ucRow,
 ...
 { 
    ...
    for(i=0;i<8;i++)
     {
         UC1701AddressSet((ucLine*2),(129-(ucRow*8)-i));//设置起始地址
        uctemp=Ascii[k+i];
